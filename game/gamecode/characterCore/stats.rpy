...
init python:
    ...
    class StatusEffects:
        ...
        def hasThisStatusEffect(self, Name):
            ...
            if Name == "Paralysis":
                if self.paralysis.potency > 0 and self.paralysis.duration > 0:
                    return True
            if Name == "Paralyzed":
        ...
        def hasThisStatusEffectPotency(self, Name, Potency):
            inverse = 1
            if Potency < 0: # Potency is the one being checked to beat, no idea why i called it that.
                inverse = -1
            ...
            if Name == "Damage":
                ...
                for each in self.tempAtk:
                    if each.duration > 0:
                        totalPotency += each.potency
                if inverse*totalPotency >= inverse*Potency:
                    return True
            if Name == "Defence":
                totalPotency = 0
                for each in self.tempDefence:
    ...
    class Skill:
        def __init__(self, name="blank", costDisplay="0",  costType="ep", skillType="attack", statType="", skillTags=[], fetishTags=[],
                        ...
                        learningCost=0, requiredStat=0, requiredLevel=1, statusEffectScaling=100, scalesWithStatusScale="", flatSFFlatScaling=0, flatSFPercentScaling=0, totalSFPercentScaling=0, unusableIfTargetHasTheseSets=[], stanceConditions=[], statusStacks=0, cost=0, isSkill="True"):
            self.name = name
