init python:
    ...
    def Resting(player):
        global favorPool, favorStrain
        spiritRegained = 1
        energyRegained = 0.5
        arousalRegained = 0.5
        if difficulty == "Hard":
            favorStrainRecovery = 25
        else:
            favorStrainRecovery = 50
        if difficulty == "Easy":
            favorPoolRecovery = 1
        else:
            favorPoolRecovery = 0 
        ...
        for perk in player.perks:
            ...
            while  p < len(perk.PerkType):
                ...
                if perk.PerkType[p] == "RestStrainRestored":
                    favorStrainRecovery += 25*(perk.EffectPower[p])
                if perk.PerkType[p] == "RestFavorRestored":
                    favorPoolRecovery += perk.EffectPower[p]
                p += 1
...
label Adventure:
    ...
    show screen ON_MapMenu
    show screen ON_MapMenuButtons
...
label RandomizeAdventure:
    ...
    $ MakeDeckSize = LocationDatabase[targetLocation].MinimumDeckSize
    $ cardCycle = 1
...
label shuffleExploration:
    ...
    $ e = 0
    $ placedRestSpots = 0
    $ avgRestPoints = preTreasureLength//3
    if avgRestPoints > 1:
        $ avgRestPoints = renpy.random.randint(1, int(avgRestPoints))
...
label explore:
    ...
    while e  < len(explorationDeck):
        ...
        hide screen ON_HealthDisplayBacking
        hide screen ON_HealthDisplay
        show screen ON_HealthDisplayBacking #onlayer master
        show screen ON_HealthDisplay #onlayer master
        $ RanAway = "False"
        $ SceneCharacters = []
        $ displayingScene = []
        $ lineOfScene = 0
