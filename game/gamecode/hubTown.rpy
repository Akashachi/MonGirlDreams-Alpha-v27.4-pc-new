...
label TimeEvent(CardType, LoopedList):
    ...
    if DataLocation >= 0:
        ...
        $ HoldingLineForTimeArray.append(copy.copy(lineOfScene))
    ...
    $ currentChoice = copy.copy(specifyCurrentChoice)

    if LoopedListHolder == []:
        $ LoopedListHolder.append(copy.deepcopy(LoopedList))
        $ holdChoiceForLoop.append(copy.copy(currentChoice))
    if teiHold != -1:
...
label Bed:
    ...
    call EnterTownLocation from _call_EnterTownLocation_4
...
label RestoreSensitivity:
    ...
    if theCost > 0:
        ...
        window hide
        $ Speaker = Goddess
        $ LastLine = "Oh, chosen hero... What do you need restored? With a donation of eros, I can remove the effects on your body from your losses. (You have [PlayerMoney] eros.)"
...
label PurgeFetishes:
    ...
    while i < len(TempFetishes):
        if i < len(TempFetishes):
            if TempFetishes[i].Type == "Fetish":
                    ...
                    if TempFetishes[i].Level*costPerFetish > 0:
                        if bufferF == 0:
                            if display1 == "" :
                                $ display1 = display
    ...
    if theCost > 0:
        ...
        window hide
        $ Speaker = Goddess
        $ LastLine = "Oh chosen hero... If you donate enough eros, I can remove the fetishes you have gained from your trials. (You have [PlayerMoney] eros.)"
        show screen fakeTextBox
...
label DonateToGoddess:
    $ donateStr = renpy.input(_("How much do you want to donate? (You have [PlayerMoney] eros.)"), length=20) or _("0")
    python:
        try:
