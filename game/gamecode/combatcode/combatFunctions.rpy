...
init python:
    ...
    def getDamageReduction(defender, Damage):
        if defender.stats.Willpower >= -100:
            damageReturn = 150.0/(150.0 + float(defender.stats.Willpower) - 5.0)
        else: 
            damageReturn = 3.33333
        extraReduction = 0
...
label combatActionTurn:
    ...
    $ startingAttacker = attacker
    $ startingDefender = defender
    ...
    if canUse == False:
        ...
        if goAttack == 1:
            ...
            while atkLo < loop:
                ...
                $ narrator.add_history(kind='adv', who="", what=renpy.substitute(_('[historyDisplay!i]'))) #doesnt actually go here because it'd be shown before the skill in history
                $ finalDamage = 0   
                $ recoilHitCheck = recoilHit
                $ recoilHit = 0
                $ statusEffectiveText = ""
    ...
    if recoilHitCheck >= 1 and player.statusEffects.surrender.duration <= 0 and currentTarget.stats.sp != 0 :
    ...
    if recoilHitCheck >= 1 and player.statusEffects.surrender.duration <= 0:
        $ orgasmTarget = theLastAttacker
        $ orgasmCauser = currentTarget
...
label theOrgasmCheck:
    ...
    while LossCheck < len(orgasmArray):
        ...
        if display != "" and edging == 0 and ignoreOrgasm == 0 and spiritLost > 0:
            ...
            $ holder = PostOrgasmCheck(orgasmArray[LossCheck], orgasmCauser)
            $ orgasmArray[LossCheck] = holder[0]
            $ orgasmCauser = holder[1]
            $ display = holder[2]
                ...
                if canGo == 0:
                    ...
                    if each.lineTrigger == "OffenceCounter":
                        ...
                        if skillChoice.skillType == "Healing" or skillChoice.skillType == "HealingEP" or skillChoice.skillType == "HealingSP" or skillChoice.skillType == "StatusHeal" or skillChoice.name == "Run Away" or skillChoice.name == " " or skillChoice.name =="Struggle"  or skillChoice.name =="Defend" or skillChoice.name =="Wait" or skillChoice.name == "Push Away":
                            ...
                            counterit = 0
                            if skillChoice.statusPotency <= 0 and skillChoice.statusEffect != "Analyze":
                                counterit = 1
...
label onRecoilTalk(defender, attacker, skillChoice):
    python:
        if attacker.species == "Player" and recoilHitCheck > 0:
...
label combatFunctions:
    python:
        ...
        def PostOrgasmCheck(theTarget, attacker):
            display = ""
        ...
        def AttackCalc(attacker, theTarget, move, autoHits=0):
            ...
            if move.recoil > 0:
                ...
                recoil = (getDamageEstimate(attacker, move)*recoilpercent + (theTarget.stats.Allure*allureFlatScaling)*recoilAllureBoost*0.01)*(fetishAtkMod*0.01)
            ...
            if renpy.random.randint(0,100) + attackerAcc + move.accuracy >= TargetsEvade - surrenderMod - restrainedMod - fetishMod:
                ...
                if move.statusOutcome != "IgnoreAttack":
                    ...
                    if effectivenessTotal >= 1.25:
                        effectiveText = "Weakspot" #"{i}{color=#ff587d}Weakspot!{/color}{/i} "
                        effectiveTextHistory = "{i}{color=#ff587d}Weakspot!{/color}{/i} "
                    elif effectivenessTotal <= 0.75:
                        effectiveText = "Frigid" #"{i}{color=#535F98}Frigid!{/color}{/i} "
                        effectiveTextHistory = "{i}{color=#535F98}Frigid!{/color}{/i} "
                    ...
                    # Somewhere this gets turned into a float again.
                    minRange = int(move.minRange)
                    maxRange = int(move.maxRange)

                    finalDamage *= renpy.random.randint(minRange, maxRange) * 0.01 #random modifyer to damage
                    ...
                    if skillChoice.statusChance > 0 and theTarget.species != "Player":
                        ...
                        if 100 <= getStatusEffectRes(theTarget, move, autoHits) - getStatusEffectAccuracy(attacker, move):
                            statusEffectiveText = "Effect Immune!" #"{b}{color=#535F98}Effect Immune!{/color}{/b}"
                            statusEffectiveTextHistory = "{b}{color=#535F98}Effect Immune!{/color}{/b}"
                        elif 0 < (theTarget.resistancesStatusEffects.getRes(move.statusEffect)):
                            statusEffectiveText = "Effect Resistant!" #"{color=#535F98}Effect Resistant!{/color}"
                            statusEffectiveTextHistory = "{color=#535F98}Effect Resistant!{/color}"
                        elif 0 > (theTarget.resistancesStatusEffects.getRes(move.statusEffect)):
                            statusEffectiveText = "Effect Weak!" #"{color=#ff587d}Effect Weak!{/color}"
                            statusEffectiveTextHistory = "{color=#ff587d}Effect Weak!{/color}"


                    display = move.outcome #+ " " + "[EffectiveText]" + "[CritText]" + "[TargetName] is aroused by " + "[FinalDamage]!" + " {i}[StatusEffectiveText]{/i}" do not remove, may be needed for normal display at some point.
                    historyDisplay = effectiveTextHistory + critText + targetName + " is aroused by " + str(finalDamage) + "! " + statusEffectiveTextHistory
                    #narrator.add_history(kind="adv", who="", what="[historyDisplay!i]") 

                    if move.recoil > 0:
                        historyDisplay += " " + attackerName + " is also aroused by " + str(recoil) +"!"
        ...
        def statusAfflict(theTarget, move, attacker=Monster(Stats(), 0, "FAKEtARGET")):
            ...
            if move.statusEffect == "Aphrodisiac":
                ...
                for perk in attacker.perks:
                    p = 0
                    while  p < len(perk.PerkType):
                        if perk.PerkType[p] == "AphrodisiacPotencyBoost":
                            posionAmp += (perk.EffectPower[p])*0.01
                        p += 1
                theTarget.statusEffects.aphrodisiac.potency += move.statusPotency*posionAmp
        ...
        def HealCalc(attacker, move):
            ...
            if(move.skillType != "StatusHeal"):
                ...
                finalHealing *= (renpy.random.randint(int(move.minRange), int(move.maxRange))*0.01) #random modifyer to damage
