...
label combat:
    ...
    if player.statusEffects.defend.potency == 0:
        $ player.statusEffects.defend.potency = 3
    ...
    if overrideCombatMusic == 0:
        $ BGMlist = []
        if IfTime("Night") == 1:
            $ BGMlist.append("music/Battle/Goodbye.mp3")
        else:
    ...
    #ensure all the names and hp bars are correct
    python:
        for each in monsterEncounter:
...
label combatEnemies:
    ...
    hide screen ON_CombatMenu
    window show
    show screen ON_CombatMenuTooltip onlayer screens
    jump combatDisplay
                        ...
                        for perk in monsterEncounter[m].perks:
                            ...
                            while  p < len(perk.PerkType):
                                if perk.PerkType[p] == "KissingAdverse":
                                    if MonStanceStruggle == "":
                                        MonStanceStruggle = "Making Out"
                                    else:
                                        MonStanceStruggle = "All"
                                    canUseStance = 0
                                p += 1
    ...
    if canUseStance == 0 and monsterEncounter[m].statusEffects.charmed.duration <= 0 and monsterEncounter[m].statusEffects.sleep.potency != -99: #OH GOD I CANT DO ANYTHING IN THIS STANCE
        ...
        if monsterEncounter[m].putInStance == 0:
            if renpy.random.randint(0,100) > 10:
                ...
                python:
                    if MonStanceStruggle == "" or MonStanceStruggle == "All":
                        ...
                        stanceCheckingo = 0
                        for stance in player.combatStance:
                            if stance.Stance == MonStanceStruggle:
                                stanceCheckingo = 1
                            if stanceCheckingo == 0:
                                stanceChecking += 1
                        stanceHPTotal = player.combatStance[stanceChecking].potency
                    ...
                    if MonStanceStruggle == "" or MonStanceStruggle == "All":
                        ...
                        python:
                            for stance in player.combatStance:
                                ...
                                if stanceCheckingo == 0:
                                    stanceChecking += 1
                        $ player.combatStance[stanceChecking].potency -= stanceDamageTotal
...
label combatEndTurn:
    ...
    if player.statusEffects.sleep.potency > 0:
        ...
        $ display = "Drowsiness slowly lulls [ThePlayerName] towards sleep, sapping [FinalSleepy] energy!"
        $ player.stats.BarMinMax()
    ...
    while mi < len(monsterEncounter):
        ...
        if monsterEncounter[mi].statusEffects.sleep.potency >= 1:
            ...
            $ display = "Drowsiness slowly lulls [AttackerName] towards sleep, sapping [FinalSleepy] energy!"
    ...
    if (player.statusEffects.hasThisStatusEffect("Defend")) and (player.statusEffects.defend.potency != 1):
        $ player.statusEffects.defend.potency -= 1
    elif not (player.statusEffects.hasThisStatusEffect("Defend")) and (player.statusEffects.defend.potency != 3):
        #$ player.statusEffects.defend.potency += 1
        $ player.statusEffects.defend.potency = 3
    $ player.stats.BarMinMax()
...
label combatLoss:
    ...
    #Get loss scene
    if (len(theLastAttacker.lossScenes) > 0):
    ...
    python:
        if LossExp > 0:
            ...
            for perk in player.perks:
                p = 0
                while  p < len(perk.PerkType):
                    if perk.PerkType[p] == "NoExp":
                        NoExp = True
                    elif perk.PerkType[p] == "ExpBoost":
...
label PostCombatWin:
    ...
    if noDoubleRewards == 0 and RanAway == "False":
...
label StanceStruggleGo:
    ...
    python:
        if howManyStances != "One":
            ...
            stanceHPTotal = monsterEncounter[target].combatStance[stanceChecking].potency
    ...
    if stanceHPTotal <= stanceDamageTotal:
        ...
        if howManyStances != "One":
            ...
            $ monsterEncounter[target].combatStance[stanceChecking].potency -= stanceDamageTotal
                        ...
                        for possibleStances in each.move:
                            if foundLine == 0:
                                for stance in RemovedStance:
                                    if possibleStances == stance and foundLine == 0:
...
label combatStruggleActivate(Struggler):
    ...
    if Struggler.statusEffects.restrained.duration <= 0:
        ...
        if Struggler.species != "Player":
            $ onlyOnce = 0
            python:
                for each in Struggler.combatDialogue:
                    canGo = 0
                    if each.lineTrigger == "EscapedRestraint":
                        if onlyOnce == 0:
                            onlyOnce = 1
                            display += each.theText[renpy.random.randint(-1, len(each.theText)-1)]
...
label combatRunAttempt:
    ...
    python:
        ...
        for each in monsterEncounter[0].combatDialogue:
            specifyStance = 0
            if overrideCombatMusic == 0:
                if each.lineTrigger == "PlayerRanAway":
                    lineFound = 1
                    display = each.theText[renpy.random.randint(-1, len(each.theText)-1)] + " "
    if lineFound == 1:
