...
# Shivan - WOOOOO COMPLETELY REDONE COMBAT MENU
# Shivan - looks like some shit you'd find in RPGmaker tbh lmao
# Shivan - No more pagination - everything is in a hierarchical scrollbox-based view
init python:
    ...
    damageResIcon = "StatusIcons/charmBlueIcon.png"
    statusImmuneIcon = "StatusIcons/ImmuneIcon.png"
    statusResIcon = "StatusIcons/downIcon.png"
    statusWeakIcon = "StatusIcons/upIcon.png"
    ...
    critDownIcon = "StatusIcons/CritChanceDown.png"
    ...
    class MenuItemDef():
        def __init__(self, name="NAME", tooltip="", showIf="True", sensitiveIf="True", openSubMenu="", jumpTo="", setVar="", setValue="", usedFilterType="", rawName=None, itemAmounts=[69, 69, 69]):
    ...
    def cmenu_checkSkillTag(skill, tag):
    ...
    # Top-level menu
    cmenu_main = [
        MenuItemDef(name="Caress", tooltip="[StatCheckPost]Gently caress your opponent. The most basic of skills, is unaffected by the target's sensitivities.\n{color=#F7B}Estimated Arousal: [caressEstimate]{/color}", jumpTo="combatBasicAttack", sensitiveIf="player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))", usedFilterType="Basic"),
        MenuItemDef(name="Skills", openSubMenu="Skills"),
        MenuItemDef(name="Items", tooltip="Use an item. Recovery items have greatly boosted Initiative", openSubMenu="Items", sensitiveIf="combatItems == 0"),
        MenuItemDef(name="Defend", tooltip="[StatCheckPost]Increase your defence, dodge chance, and get a bonus against stat checks.", jumpTo="combatDefend", sensitiveIf="player.statusEffects.charmed.duration <= 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))", usedFilterType="Basic"),
        MenuItemDef(name="Run", tooltip="[StatCheckPost]Attempt to run!", jumpTo="combatRun", showIf="cmenu_showRunOpt()", sensitiveIf="player.statusEffects.charmed.duration <= 0 and canRun == True and Rut == False and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"),
        MenuItemDef(name="Push Away", tooltip="[StatCheckPost]Attempt to get out of a stance! Usable for free once per turn!", setVar="stanceBreaking", setValue=1, jumpTo="pushaway", showIf="not cmenu_showRunOpt() and cmenu_showSinglePushOption()", sensitiveIf="player.statusEffects.charmed.duration <= 0 and pushAwayAttempt == 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"),
        MenuItemDef(name="Push Away", tooltip="[StatCheckPost]Attempt to get out of a stance! Usable for free once per turn!", openSubMenu="Push", showIf="not cmenu_showRunOpt() and cmenu_showPushList()", sensitiveIf="player.statusEffects.charmed.duration <= 0 and pushAwayAttempt == 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"),
        MenuItemDef(name="Push Away", tooltip="[StatCheckPost]Attempt to get out of a stance! Usable for free once per turn!", openSubMenu="PushMon", showIf="not cmenu_showRunOpt() and cmenu_showMonList()", sensitiveIf="player.statusEffects.charmed.duration <= 0 and pushAwayAttempt == 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"),
    ...
    # Skill subcategories
    cmenu_skills = [
        MenuItemDef(name="Esc Skills", openSubMenu="SkillList", setVar="moveFilter", setValue="Escape", showIf="cmenu_hasEscapeSkills() and player.statusEffects.restrained.duration > 0"),
        ...
        MenuItemDef(name="Magic", openSubMenu="SkillList", setVar="moveFilter", showIf="player.statusEffects.restrained.duration <= 0", setValue="Magic"),
        MenuItemDef(name="Pain", openSubMenu="SkillList", setVar="moveFilter", showIf="player.statusEffects.restrained.duration <= 0", setValue="Pain"),
        ...
        MenuItemDef(name="Holy", openSubMenu="SkillList", setVar="moveFilter", showIf="player.statusEffects.restrained.duration <= 0", setValue="Holy")
        ]
    ...
    # Struggle options
    cmenu_struggle = [
        MenuItemDef(name="Struggle", tooltip="[StatCheckPost]Try to get free!", sensitiveIf="player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))", jumpTo="combatStruggle"),
        MenuItemDef(name="Struggle!!!", tooltip="[BigStruggleDisplay]Desperately try to get free! About twice as effective as normal struggle.", setVar="desperateStruggle", setValue=1,  jumpTo="combatStruggle", sensitiveIf="player.stats.ep >= int(math.floor(20*GetParalEnergyChange(player)+GetParalFlatEnergyChange(player)))"),
        MenuItemDef(name="Skills", openSubMenu="Skills"),
        MenuItemDef(name="Defend", tooltip="[StatCheckPost]Increase your defence, and get a bonus against stat checks!", jumpTo="combatDefend", sensitiveIf="player.statusEffects.charmed.duration <= 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"),
    ...
    def skillIsVisible(skill):
        ...
        if skill.requiresStance == "Any":
            visible = True
            if player.statusEffects.restrained.duration > 0 and skill.statusEffect != "Escape" and cmenu_checkSkillTag(skill, "RestrainUsable") == False:    
                    visible = False
        else:
            for stanceCheck in player.combatStance:
    ...
    def skillIsUsable(skill, user):
        if user.species == "Player":
            if GetParalFlatEnergyChange(user) > 0: 
                if user.stats.ep < int(math.floor(GetParalFlatEnergyChange(user))):
                    return False
            if skill.costType == "ep":
                if user.stats.ep < int(math.floor(skill.cost*GetParalEnergyChange(user)+GetParalFlatEnergyChange(user))):
                    return False
            elif skill.costType == "sp":
                if user.stats.sp < skill.cost:
                    return False

          
        noGo = False
        canGo = False
        for Pstance in user.combatStance:
            for stances in skill.startsStance:
                if Pstance.Stance == stances: # restrict stance we're already in - no equivalence
                    noGo = True
                if user.statusEffects.restrained.duration > 0 and cmenu_checkSkillTag(skill, "RestrainUsable") == False:    
                    if stances != "":
                        noGo = True
                        
            if user.statusEffects.restrained.duration > 0 and cmenu_checkSkillTag(skill, "RestrainUnusable") == True:
                noGo = 0

            if user.statusEffects.restrained.duration > 0 and cmenu_checkSkillTag(skill, "RestrainUsable") == False:
                if skill.statusEffect == "Restrain" or skill.statusEffect == "EventRestrain":   
                    noGo = True
            ...
            if "Any" != skill.requiresStance:
                ...
                if user.statusEffects.restrained.duration > 0 and skill.statusEffect != "Escape" and cmenu_checkSkillTag(skill, "RestrainUsable") == False:    
                    noGo = True
                canGo = True
        ...
        if skill.requiresStatusEffectSelf != "" and skill.requiresStatusEffectSelf != "None":
            if user.statusEffects.hasThisStatusEffect(skill.requiresStatusEffectSelf) == False:
                noGo = True
        ...
        if skill.requiresStatusPotencySelf != 0:
            if user.statusEffects.hasThisStatusEffectPotency(skill.requiresStatusEffectSelf, skill.requiresStatusPotencySelf) == False:
                noGo = True
        ...
        for skillStatus in skill.unusableIfStatusEffectSelf:
            if user.statusEffects.hasThisStatusEffect(skillStatus) == True:
                noGo = True
    ...
    def skillIsUsableForTarget(skill, target):
        ...
        for skillStatus, skillPotency in itertools.zip_longest(skill.unusableIfStatusEffect, skill.unusableIfStatusPotency, fillvalue=0):
            if skillPotency != 0:
                if (target.statusEffects.hasThisStatusEffectPotency(skillStatus, skillPotency)) and (target.statusEffects.hasThisStatusEffect(skillStatus)):
                    noGo = True
            else:
                if target.statusEffects.hasThisStatusEffect(skillStatus):
                    noGo = True
        try:
            skill.requiresPerk
        except:
    ...
    def cmenu_getSkillList():
        ...
        for skill in player.skillList:
            if skillIsVisible(skill):
                usable = skillIsUsable(skill, player)
    ...
    def getSkillToolTip(skill, player, skillDes):
        ...
        if skill.costType != "ep" or skill.cost == 0:
            skillToolTip += GetParalFlatEnergyCostDisplay(player)
        ...
        if skill.cost != 0:
            ...
            if skill.costType == "ep":
                actualCost = int(math.floor(actualCost*GetParalEnergyChange(player)+GetParalFlatEnergyChange(player)))
                if player.statusEffects.paralysis.duration > 0:
                    skillToolTip += "{color=#F8FF6A}Costs " + str(actualCost) + " Energy.{/color} "
                else:
                    skillToolTip += "{color=#7DF}Costs " + str(actualCost) + " Energy.{/color} "
            elif skill.costType == "hp":
                actualCost = skill.cost + (player.stats.max_true_hp-100)*0.15
                actualCost = math.floor(actualCost)
                actualCost = int(actualCost)
                skillToolTip += "{color=#ff587d}Costs " + str(actualCost) + " Arousal.{/color} "
            else:
                skillToolTip += "Costs " + str(actualCost) + " " + costDisplay + ". "
        if skill.statType != "" and skill.statType != "None" and skill.statType != "PercentMaxArousal" and skill.statType != "PercentMaxEnergy":
        ...
        if skill.skillType == "attack":
            ...
            if skill.recoil > 0:
                ...
                recoilpercent = (skill.recoil*0.01)*(recoilPercentDecreace*0.01)
                recoil = (getDamageEstimate(player, skill))*recoilpercent
                recoil =  int(math.floor(recoil))

                skillToolTip += "\n{color=#F7B}Estimated Arousal: " + str(damageEstimate) + "   Estimated Recoil: " + str(recoil) + "{/color}"
            else:
        ...
        if skill.statusEffect == "Damage" or skill.statusEffect == "Defence" or skill.statusEffect == "Crit" or skill.statusEffect == "Power" or skill.statusEffect == "Technique" or skill.statusEffect == "Intelligence" or skill.statusEffect == "Willpower" or skill.statusEffect == "Allure" or skill.statusEffect == "Luck" or skill.statusEffect == "%Power" or skill.statusEffect == "%Technique" or skill.statusEffect == "%Intelligence" or skill.statusEffect == "%Willpower" or skill.statusEffect == "%Allure" or skill.statusEffect == "%Luck":
            ...
            if buff == "Buff: ":
                ...
                if postNote =="":
                    skillToolTip += "\n{color=#F7B}Estimated " + skill.statusEffect +" Debuff: " + str(damageEstimate) + "{/color}"
                else:
                    skillToolTip += "\n{color=#F7B}Estimated " + skill.statusEffect +" Debuff: " + str(damageEstimate) + "%{/color}"
        ...
        for i, item in enumerate(player.inventory.items):
            if item.itemType == "Consumable" or item.itemType == "DissonantConsumable" or item.itemType == "CombatConsumable":
                ...
                if len(item.skills) > 0:
                    ...
                    usable = skillIsUsable(skillToCheck, player)
                ...
                arr.append(MenuItemDef(name=display, tooltip=itemToolTip, setVar="ItemNumber", setValue=i+1, jumpTo="combatItemChoice", sensitiveIf=usable, usedFilterType="Item", rawName=item.name, itemAmounts=[item.ep, item.hp, item.sp]
                ))
    ...
    def cmenu_getPushMonList():
        ...
        for i, mon in enumerate(monsterEncounter):
            ...
            if len(mon.combatStance) == 1 and mon.combatStance[0].Stance != "None":
                ...
                arr.append(MenuItemDef(name=name, tooltip="[StatCheckPost]Try to remove the " + stanceName + " stance with " + name + ".", setVar="target", setValue=i, jumpTo="combatPushAway", sensitiveIf="player.statusEffects.charmed.duration <= 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"))

            elif len(mon.combatStance) > 1:
                arr.append(MenuItemDef(name=name, tooltip="[StatCheckPost]Try to remove stances with " + name + ".", setVar="target", setValue=i, openSubMenu="Push", sensitiveIf="player.statusEffects.charmed.duration <= 0 and player.stats.ep >= int(math.floor(GetParalFlatEnergyChange(player)))"))
    ...
    def cmenu_getPushList():
        ...
        for sti, stance in enumerate(monsterEncounter[target].combatStance):
            name = stance.Stance
            monName = monsterEncounter[target].name
            arr.append(MenuItemDef(name=name, tooltip="Try to remove the " + name + " stance with " + monName + ".", setVar="tryRemoveThisStance", setValue=sti, jumpTo="removeThisStanceStances"))

        arr.append(MenuItemDef(name="All stances", tooltip="Try to remove all stances with " + monName + ".", jumpTo="removeAllStances"))
        return arr
    ...
    def cmenu_refreshMenu():
        if len(cmenu_columns) >= 3 and cmenu_breadcrumb[0] == "Skills":
...
transform zoomBar:
    subpixel True
    zoom 0.8
transform zoomBarBounce(child):
    child
    subpixel True
    zoom 0.85
    linear 0.1 zoom 0.8
transform zoomBarShrink(child):
    child
    subpixel True
    zoom 0.75
    linear 0.1 zoom 0.8
# Rotate the bar after cropping
transform rotateBar(numby):
    subpixel True
    align (0.5, 0.5)
    rotate numby
# Anchor EP
...
screen ON_HealthDisplay(mainMenu=False):
    ...
    use StatusBar(player, mainMenu) #it's in on_enemyCardScreen.rpy
    if mainMenu == False:
        use ON_CombatMenuTooltip

screen ON_HealthDisplayBacking():
    zorder 0
    add "gui/playerbackUnderSide.png" xpos 0 ypos 660-10 alpha 0.9 # Add a second bg img in case textbox.png gets hidden
...
init python:
    ...
    barChecked = False
    barAnimate = False
    def checkBarAnimate(theLast=False):
        global barAnimate
        if theLast == False:
            barAnimate = True
        if barAnimate == True and theLast == True:
            barAnimate = False

    def bouncyBouncy(d):
        global previousEP, previousHP, previousSP, barChecked, barAnimate
        if persistent.animatedUI:
            if player.stats.hp > player.stats.max_true_hp and previousHP != player.stats.hp:
                barChecked = not barChecked
                if not barAnimate:
                    previousHP = player.stats.hp
                return At(d, zoomBarBounce)
            elif previousSP != player.stats.sp and previousHP != player.stats.hp and previousSP != 0:
                barChecked = not barChecked
                if not barAnimate:
                    previousSP = player.stats.sp
                return At(d, zoomBarShrink)
            #     previousEP = player.stats.ep
            #     barChecked = not barChecked
            # if previousSP != player.stats.sp:
            #     previousSP = player.stats.sp
            #     barChecked = not barChecked
            barChecked = not barChecked
            return At(d, zoomBar)
...
screen ON_HealthDisplayInner(mainMenu=False, xOffset=0, menuCall=0):
    zorder 100
    if not mainMenu:
        add "gui/playerback.png" xpos xOffset-958 ypos -18 # Add a second bg img in case textbox.png gets hidden
    else:
        add "gui/healthBackTotal.png" xpos xOffset-958 ypos -18
    ...
    if statusEffectiveText != "":
        $ positioningOfText += 1
    if attacker.species != "Player" and (finalDamage > 0 or statusEffectiveText != ""):     
        $ lastPlayerDamage = finalDamage
        $ lastPlayerDamageCrit = critText
        $ lastPlayerDamageWeak = effectiveText
        $ lastPlayerStatusRes = statusEffectiveText
        use CardDamageNumberDisplayActual(0, finalDamage, critText, effectiveText, statusEffectiveText, "in", positioningOfText, xOffset)      
            
    else:
        use CardDamageNumberDisplayActual(0, lastPlayerDamage, lastPlayerDamageCrit, lastPlayerDamageWeak, lastPlayerStatusRes, "out", positioningOfText, xOffset)

    if recoilHit > 0 and attacker.species == "Player": 
        $ lastPlayerRecoil = recoilHit
        use CardDamageNumberDisplayActual(0, recoilHit, "", "", "", "in", 0, xOffset)
    else:
        use CardDamageNumberDisplayActual(0, lastPlayerRecoil, "", "", "", "out", 0, xOffset)
        

    # Arousal Bar
    fixed:
        xpos xOffset - 20 ypos 60
        xsize 1 ysize 1
        bar:
            left_bar "gui/HPBuffer.png"
            right_bar Frame("gui/HPBack.png")
            value (max(0.043, player.stats.hp) if player.stats.hp > 0 else 0)
            range player.stats.max_true_hp
            if player.stats.hp != previousHP:
                changed checkBarAnimate()
            xsize 225 ysize 215
            at [bouncyBouncy, rotateBar(33), mirrorAndAnchorAP]
            alt ""
        bar:
            left_bar "gui/HP.png"
            right_bar Frame("gui/HPBack.png")
            if persistent.animatedUI:
                value AnimatedValue(max(0.043, player.stats.hp / player.stats.max_true_hp) if player.stats.hp > 0 else 0, delay=0.34)
            else:
                value (max(0.043, player.stats.hp) if player.stats.hp > 0 else 0)
                range player.stats.max_true_hp
            if player.stats.hp != previousHP:
            ...
            alt ""
        bar:
            left_bar "gui/HPBar.png"
            right_bar "gui/HPBar.png"
            value AnimatedValue(max(0.043, player.stats.hp / player.stats.max_true_hp) if player.stats.hp > 0 else 0, delay=0.34)
            if player.stats.hp != previousHP:
                changed checkBarAnimate(True)
            xsize 239 ysize 240
            xoffset 10
            at [bouncyBouncy, rotateBar(33), mirrorAndAnchorAP]
            alt ""
    ...
    fixed:
        ...
        xsize 1 ysize 1
        bar:
            left_bar "gui/MPBuffer.png"
            right_bar "gui/HPBack.png"
            value (max(0.043, player.stats.ep) if player.stats.ep > 0 else 0)
            range player.stats.max_true_ep
            xsize 225 ysize 215
        ...
        bar:
            ...
            right_bar "gui/HPBack.png"
            if persistent.animatedUI:
                value AnimatedValue(max(0.043, player.stats.ep / player.stats.max_true_ep) if player.stats.ep > 0 else 0, delay=0.34)
            else:
                value(max(0.043, player.stats.ep) if player.stats.ep > 0 else 0)
                range player.stats.max_true_ep
            xsize 225 ysize 215
    ...
    fixed:
        ...
        alt ""
        at rotateEPText
        xpos xOffset + 88 ypos -80

    # Spirit Bar
    vbar:
        if persistent.modsArePresent == True:
            bottom_bar "gui/SpiritBufferMod.png"
        else:
            bottom_bar "gui/SpiritBuffer.png" 
        top_bar "gui/SpiritEmpty.png"
        value (max(0.043, player.stats.sp) if player.stats.sp > 0 else 0)
        range player.stats.max_true_sp
        xsize 77 ysize 70
        xcenter xOffset ypos 27
        # hover_alt "Remaining arousal: [player.stats.max_true_hp-player.stats.hp], remaining energy: [player.stats.ep], remaining spirit: [player.stats.sp]"
    vbar:
        if persistent.modsArePresent == True:
            bottom_bar "gui/SpiritFillMod.png"
        else:
            bottom_bar "gui/SpiritFill.png" 
        top_bar "gui/SpiritEmpty.png"
        xsize 77 ysize 70
        if persistent.animatedUI:
            value AnimatedValue(max(0.043, player.stats.sp/(player.stats.max_true_sp+0.0)) if player.stats.sp > 0 else 0, delay=0.34)
        else:
            value (max(0.043, player.stats.sp) if player.stats.sp > 0 else 0)
            range player.stats.max_true_sp
        xcenter xOffset ypos 27
    ...
    if cmenu_showHealthTooltip:
        text "{color=#DFD}[player.stats.sp]/[player.stats.max_true_sp]{/color}":
            size 25 xcenter xOffset ypos 46
            alt ""
    else:
        text "{color=#DFD}[player.stats.sp]{/color}":
            size 25 xcenter xOffset ypos 46
            # alt "Press ? for current arousal, energy, and sp."
            alt ""
    # Add status effects over player icon
...
screen ON_EnemyCardScreen():
    ...
    if hidingCombatEncounter == 0:
        if RoledCGOn == 0:
            ...
            for mC, mon in reversed(list(enumerate(monsterEncounter))):
                $ placement = PlacementFunction(mC, monsterEncounter, xAdjustBase, alternating)
                use EnemyCardCG(mC, mon, placement[0], placement[1])
        else:
            for mC, mon in reversed(list(enumerate(monsterEncounterCG))):
                $ placement = PlacementFunction(mC, monsterEncounterCG, xAdjustBase, alternating)
                use EnemyCardCG(mC, mon, placement[0], placement[1])

        if RoledCGOn == 0:
            for mC, mon in reversed(list(enumerate(monsterEncounter))):
                $ placement = PlacementFunction(mC, monsterEncounter, xAdjustBase, alternating)
                use EnemyCardUI(mC, mon, placement[0], placement[1])
        else:
            $ extraCGMons = 1
            for mC, mon in list(enumerate(monsterEncounter)):
                if combatMonUIoveride[mC][0] == 0.0 and combatMonUIoveride[mC][1] == 0.0:
                    use EnemyCardUICG(extraCGMons, mon, placement[0], placement[1])
                    $ extraCGMons += 1

            for mC, mon in reversed(list(enumerate(monsterEncounter))):
                if combatMonUIoveride[mC][0] != 0.0 or combatMonUIoveride[mC][1] != 0.0:
                    use EnemyCardUIOverride(mC, mon, combatMonUIoveride[mC][0], combatMonUIoveride[mC][1])


# Tooltip screen for combat menu
screen ON_CombatMenuTooltip():
    $ tt = GetTooltip()
    $ ttContents = ""
    $ restrainedMonster = 0
    zorder 100
    if tt or ttCombat or ttTarget: 
        $ caressEstimate = getDamageEstimate(player, getSkill("Caress", SkillsDatabase)) 
        ...
        frame:
            ...
            vbox:
                spacing 5
                if tt:
                    text "[tt!i]" xsize 825 size 26
                    $ ttContents = tt
                elif ttCombat:
                    text "[ttCombat!i]" xsize 825 size 26
                    $ ttContents = ttCombat
                elif ttTarget:
                    text "[ttTarget!i]" xsize 825 size 26
                    $ ttContents = ttTarget
                else:
                    text ""

                if ttContents.endswith("Attempt to get out of a stance! Usable for free once per turn!"):
                    # When there is one stance and one monster
                    if len(player.combatStance) == 1 and len(monsterEncounter) == 1:
                        hbox:
                            text "Stance HP: " size 26 yoffset -5
                            bar:
                                style "bar" 
                                xsize 225 ysize 26
                                value monsterEncounter[0].combatStance[0].potency
                                range monsterEncounter[0].combatStance[0].maxPotency
                                alt ""
                    # When there is multiple stances and one monster
                    elif len(player.combatStance) > 1 and len(monsterEncounter) == 1:
                        $ allPotency = 0
                        $ allMaxPotency = 0
                        for i, monster in enumerate(monsterEncounter):
                            for each in monsterEncounter[i].combatStance:
                                $ allPotency += each.potency
                                $ allMaxPotency += each.maxPotency
                        hbox:
                            text "All Stance HP: " size 26 yoffset -5
                            bar:
                                style "bar" 
                                xsize 225 ysize 26
                                value allPotency
                                range allMaxPotency
                                alt ""
                if "Try to remove" in ttContents:
                    for i, monster in enumerate(monsterEncounter):
                        if monster.name in ttContents:
                            if "stances" in ttContents:
                                $ allPotency = 0
                                $ allMaxPotency = 0
                                for each in monster.combatStance:
                                    $ allPotency += each.potency
                                    $ allMaxPotency += each.maxPotency
                                hbox:
                                    text "All Stance HP: " size 26 yoffset -5
                                    bar:
                                        ...
                                        range allMaxPotency
                                        alt ""
                            elif "stance" in ttContents:
                                for each in monster.combatStance:
                                    if each.Stance in ttContents:
                                        hbox:
                                            text "Stance HP: " size 26 yoffset -5
                                            bar:
                                                style "bar" 
                                                xsize 225 ysize 26
                                                value each.potency
                                                range each.maxPotency
                                                alt ""
                elif ttContents.startswith("Restrained! Heavily limits") or ttContents.endswith("Desperately try to get free! About twice as effective as normal struggle.") or ttContents.endswith("Try to get free!"):
                    if not player.statusEffects.restrained.duration > 0:
                        for monster in monsterEncounter:
                            if monster.statusEffects.restrained.duration > 0:
                                $ restrainedMonster = monster.statusEffects.restrained.duration
                                break
                            else:
                                $ restrainedMonster = 0
                    hbox:
                        text "Restraint HP: " size 26 yoffset -5
                        bar:
                            style "bar" 
                            xsize 225 ysize 26
                            if player.statusEffects.restrained.duration > 0:
                                value player.statusEffects.restrained.duration
                            else:
                                if restrainedMonster != 0:
                                    value restrainedMonster
                            range restraintMaxHP
                            alt ""
...
screen ON_MenuItem(i, depth, inMainMenu=False):
    ...
    python:
        ...
        Sensitivechecker = skillIsUsable(player.skillList[getFromName(i.name, player.skillList)], player)
    ...
    if cmenu_eval(i.showIf):
        hbox:
            ...
            button:
                ...
                hovered [SetVariable("itemEnergyAmount", i.itemAmounts[0]), SetVariable("itemArousalAmount", i.itemAmounts[1]), SetVariable("itemSpiritAmount", i.itemAmounts[2])]
                # Breadcrumb items close their submenus when clicked
                if breadcrumb:
                    action If(disabled, false=[
                        SetVariable("ttCombat", ""),
                ...
                # Else, show tooltips, use healing skills from menu
                    ...
                    if i.openSubMenu:
                        ...
                        if manualSort != "Skills":
                            if(player.skillList[getFromName(i.name, player.skillList)].skillType == "Healing" or player.skillList[getFromName(i.name, player.skillList)].skillType == "HealingEP" or player.skillList[getFromName(i.name, player.skillList)].skillType == "HealingSP" or player.skillList[getFromName(i.name, player.skillList)].skillType == "StatusHeal") and (player.skillList[getFromName(i.name, player.skillList)].statusOutcome != "CombatOnly"):
                                action [SensitiveIf(Sensitivechecker), Show("ON_SkillPrompt", useSkillTarget=getFromName(i.name, player.skillList))]
                            else:
                                action NullAction()
                        elif manualSort == "Skills":
                            action [SetVariable ("skillTarget", getFromName(i.name, player.skillList)), Function(SetSkillOrder)]
...
screen ON_CombatMenu:
    key "K_c" action [
        ...
        SetVariable("useSkill", 0),
        SetVariable("skillTarget", 0),
    ...
    add "gui/textbox.png" xalign 0.5 yalign 1.0 alpha 0.86
