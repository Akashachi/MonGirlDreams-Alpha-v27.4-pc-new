init -1 python:
    def play_sfx(soundEffect): #WAS FOR EXPERIMENTAL FEATURE IGNORE THIS
        ...
        if soundEffect == "QuickKiss":
            ...
            usingBank = 1
        if usingBank == 0:
            psfx = copy.copy(displayingScene.theScene[lineOfScene])
        else:
    ...
    def bgToNightDay(bg, parse, addIn):
        ...
        if renpy.loadable(bgCheck, "images"):
            bg = copy.copy(bgCheck)
        return bg
    ...
    def perkDurationDisplay(description, duration, timeType):
        ...
        if parsed[1] == "|perkDuration|":
            ...
            if timeType == "TurnDuration":
                ...
                if duration > 6:
                    timeDura = "Lasts " + str(int(duration/6)) + " more days."
                else:
    ...
    def getFromName(theName, searchThis):
        if searchThis is SkillsDatabase:
            return SkillsDatabaseDict.get(theName, -1)
        elif searchThis is PerkDatabase:
            return PerkDatabaseDict.get(theName, -1)
        elif searchThis is ItemDatabase:
            return ItemDatabaseDict.get(theName, -1)
        elif searchThis is MonsterDatabase:
            return MonsterDatabaseDict.get(theName, -1)
        elif searchThis is EventDatabase:
            return EventDatabaseDict.get(theName, -1)
        elif searchThis is FetishList:
            return FetishDatabaseDict.get(theName, -1)
        elif searchThis is AdventureDatabase:
            return AdventureDatabaseDict.get(theName, -1)
        elif searchThis is LocationDatabase:
            return LocationDatabaseDict.get(theName, -1)
    ...
    def CalcGoddessFavor(player):
        if difficulty == "Hard":
            favor = 0
        else:
            favor = 1

        favor += int(math.floor((player.stats.getStat("Luck") - player.getStatBonusReduction("Luck"))*0.05))

        for perk in player.perks:
            p = 0
            while  p < len(perk.PerkType):
                if perk.PerkType[p] == "FavorBoost":
                    favor += perk.EffectPower[p]
                p += 1
